...
function contactform7_before_send_mail( $form_to_DB )
{
    ...
    var_dump($order_title); exit;
    //$author =$formData['author'];
    $content = $formData['content'];
    $datapicker = $formData['datapicker'];
    $order_user_name = $formData['user_name'];
    $user_email = $formData['user_email'];
    $post_image = $formData['file-643'];
    //$new_content = $content . '<a href="http://localhost/poiskuslug/wp-content/uploads/' . $post_image . '"><img src="http://localhost/poiskuslug/wp-content/uploads/' . $post_image . '" width="257" height="300" class="alignnone size-medium" /></a>';

    //$the_post_id = wp_insert_post($post);


    //__update_post_meta($the_post_id, 'order_user_name', $order_user_name);

//    $wpdb->insert( 'wp_posts', array( 'post_title' => $title, 'post_content'=> $content .'<a href="http://localhost/poiskuslug/wp-content/uploads/'.$post_image.'"><img src="http://localhost/poiskuslug/wp-content/uploads/'.$post_image.'" width="257" height="300" class="alignnone size-medium" /></a>' ), array( '%s' ) );

    //update_post_meta( $the_post_id, 'order_user_name', $order_user_name );
