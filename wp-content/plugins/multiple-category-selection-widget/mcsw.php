...
{    
    if (!is_admin()) {
        ...
        {       
            ...
            foreach ($cats as $cat)
            {
                $cat = (int) $cat; // Just ensuring that nothing can be posted that's text
                if ($cat == 0)
                    unset($cats[$count]);
                else
                    $sql .= $cat.',';
...
function wpmm_modify_query($query) {
		if (isset($_SESSION['wpmm_cats']) && isset($_SESSION['wpmm_search_vars']) && !empty($query->query_vars['categories'])) {
			$query->set('category__'.$_SESSION['wpmm_search_vars'], explode(',',$_SESSION['wpmm_cats']));
		}
		if (isset($_SESSION['wpmm_title']) && !empty($query->query_vars['categories'])) {
...
class MCSWwidget extends WP_Widget {
    ...
    public function __construct() {
        parent::__construct(false, $name = 'Multi-Category');	
    }
